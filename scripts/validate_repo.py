...
def main() -> None:
    ...
    full_text = (ROOT / "notebooks/04_full_finetuning_design.ipynb").read_text(encoding="utf-8")
    if "DESIGN_ONLY = True" not in full_text or "validate_full_sft_hardware" not in full_text:
        raise RuntimeError("Notebook 04 must remain design-only and hardware-aware.")
    if "subprocess.run(full_cmd" in full_text:
        raise RuntimeError("Notebook 04 must not launch the full-SFT command.")
