    ...
    "02_local_bf16_baseline.ipynb": notebook(
        [
            ...
            markdown(
                ...
                Process isolation matters: when evaluation finishes, vLLM exits and releases GPU memory before LoRA training. Raw predictions are saved before SQL scoring and can be reused only when the model and evaluation contract match, so a scoring failure cannot waste another generation pass. One 80 GB GPU is the conservative default; set `NEMOTRON_INFERENCE_GPUS=2` only if this model/backend combination has been rehearsed on the node.
                """
            ),
            code(
                """
