...
class NotebookTests(unittest.TestCase):
    ...
    def test_local_and_both_peft_paths_use_vllm_on_matching_holdouts(self):
        ...
        self.assertIn("WORKSHOP_MODE = True", nano)
        self.assertIn("WORKSHOP_MODE = False", lightning)
        self.assertIn("TRAIN_SAMPLES = PROFILE.default_train_samples", nano)
        self.assertIn("TRAIN_SAMPLES = PROFILE.extended_train_samples", nano)
        self.assertIn("peft_{RUN_VARIANT}_text2sql.json", nano)
        self.assertIn("{PROFILE.lora_checkpoint_name}-{RUN_VARIANT}", nano)
